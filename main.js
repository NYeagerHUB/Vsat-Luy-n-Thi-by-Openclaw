/* ============================================================
   V-SAT ONLINE — Main Application
   ============================================================ */

// ---- STATE ----
const state = {
  answers:     {},   // { '1a': true/false, '10': 'A', ... }
  submitted:   false,
  currentQ:    0,    // index into EXAM_DATA.questions
};
...
function answerSub(partId, value) {
  ...
  // check if all parts answered -> auto-update nav
  const q = EXAM_DATA.questions[state.currentQ];
  if (q.type === 'truefalse' && q.parts.every(p => state.answers[p.id] !== undefined)) {
